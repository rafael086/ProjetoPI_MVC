...
namespace ProjetoPI_MVC.ViewModels
{
    public class ProjetosOngsViewModel
    {
        ...
        {
            Projetos = new ProjetoPI_MVCContext().Projetos.OrderByDescending(p => p.Id).Take(3).ToList();
            Ongs = new ProjetoPI_MVCContext().Usuarios.OfType<Ongs>().OrderByDescending(o => o.Id).Take(3).ToList();
        }
